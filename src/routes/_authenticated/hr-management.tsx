        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              الموظفون، الحضور، الإجازات، وسير الرواتب.
            </p>
          </div>
        </div>
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
          <TabsList className="mb-4">
            <TabsTrigger value="employees">
              <Users className="ms-1 h-4 w-4" /> الموظفون
            </TabsTrigger>
            <TabsTrigger value="attendance">
              <ClipboardCheck className="ms-1 h-4 w-4" /> الحضور
            </TabsTrigger>
            <TabsTrigger value="leaves">
              <CalendarDays className="ms-1 h-4 w-4" /> الإجازات
            </TabsTrigger>
            <TabsTrigger value="payroll">
              <Wallet className="ms-1 h-4 w-4" /> الرواتب
            </TabsTrigger>
          </TabsList>
    ...
    <div className="space-y-4">
      ...
      {empQ.isLoading ? (
        ...
        <EmptyPanel
          ...
          desc="أضف أول موظف لبدء إدارة الحضور والرواتب."
        />
      ) : (
              ...
              {rows.map((e) => (
                <tr key={e.id} className="border-t">
                  ...
                  <td className="p-3 text-center font-bold">
                    {Number(e.monthly_salary).toLocaleString("ar-SA")}
                  </td>
                  ...
                  <td className="p-3 text-center">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {e.is_active && (
                        <button
                          onClick={() => {
                            if (confirm("تعطيل الموظف؟")) delMut.mutate(e.id);
                          }}
      ...
      <DialogContent className="max-w-xl" dir="rtl">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <TextField
            label="الراتب الشهري"
            type="number"
            value={String(form.monthly_salary ?? 0)}
            onChange={(v) => setForm({ ...form, monthly_salary: Number(v) })}
          />
          <TextField
            label="تاريخ التعيين"
            type="date"
        ...
        <DialogFooter>
          ...
          <button
            onClick={() => mut.mutate()}
            disabled={mut.isPending || !(form.full_name ?? "").trim()}
...
function AttendanceTab() {
    ...
    mutationFn: (vars: {
      ...
      fn({
        data: {
          ...
          work_date: date,
          status: vars.status,
          check_in: vars.check_in ?? null,
          check_out: vars.check_out ?? null,
        },
      }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {attQ.isLoading || empQ.isLoading ? (
        <LoadingPanel />
      ) : emps.length === 0 ? (
        <EmptyPanel
          icon={<Users className="h-8 w-8" />}
          title="لا يوجد موظفون"
          desc="أضف موظفين أولاً."
        />
      ) : (
        <div className="overflow-hidden rounded-2xl border bg-card">
          <table className="w-full text-sm">
                  ...
                  <tr key={e.id} className="border-t">
                    ...
                    <td className="p-3 text-center">
                      <input
                        type="time"
                        defaultValue={
                          rec?.check_in ? new Date(rec.check_in).toISOString().slice(11, 16) : ""
                        }
                        onBlur={(ev) =>
                          set({
                            check_in: ev.target.value ? `${date}T${ev.target.value}:00` : null,
                          })
                        }
                    ...
                    <td className="p-3 text-center">
                      <input
                        type="time"
                        defaultValue={
                          rec?.check_out ? new Date(rec.check_out).toISOString().slice(11, 16) : ""
                        }
                        onBlur={(ev) =>
                          set({
                            check_out: ev.target.value ? `${date}T${ev.target.value}:00` : null,
                          })
                        }
                        className="rounded-md border bg-background px-2 py-1 text-sm"
                      />
                    ...
                    <td className="p-3 text-center">
                      <select
                        ...
                        onChange={(ev) => set({ status: ev.target.value as AttendanceStatus })}
                        className="rounded-md border bg-background px-2 py-1 text-sm"
                      >
...
function LeavesTab() {
  ...
  const q = useQuery({
    ...
    queryFn: () => listLeaves({ data: { status } }),
  });
  const qc = useQueryClient();
        ...
        <div className="grid gap-3">
          {(q.data ?? []).map((r) => (
            <div key={r.id} className="rounded-2xl border bg-card p-4">
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {r.leave_type} · {r.from_date} → {r.to_date} · {r.days} يوم
                  </div>
                </div>
              ...
              {r.review_notes && (
                <p className="mb-2 rounded-lg bg-muted/40 p-2 text-xs">مراجعة: {r.review_notes}</p>
              )}
              {r.status === "pending" && (
                <div className="flex gap-2">
                  <button
                    onClick={() => rev.mutate({ id: r.id, status: "approved" })}
                    disabled={rev.isPending}
                    className="inline-flex items-center gap-1 rounded-full bg-emerald-500 px-3 py-1 text-xs font-semibold text-white"
                  >
                  ...
                  <button
                    onClick={() =>
                      rev.mutate({
                        id: r.id,
                        status: "rejected",
                        notes: prompt("سبب الرفض؟") ?? undefined,
                      })
                    }
                    disabled={rev.isPending}
                    className="inline-flex items-center gap-1 rounded-full bg-destructive px-3 py-1 text-xs font-semibold text-destructive-foreground"
                  >
...
function NewLeaveDialog({
  ...
  const mut = useMutation({
    mutationFn: () =>
      fn({ data: { employee_id, leave_type, from_date, to_date, reason: reason || null } }),
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg" dir="rtl">
        ...
        <DialogFooter>
          ...
          <button
            onClick={() => mut.mutate()}
            disabled={mut.isPending || !employee_id || !from_date || !to_date}
            className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50"
          >
...
function PayrollTab() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      toast.success("تم إنشاء سير الرواتب");
    ...
    <div className="space-y-4">
      ...
      {q.isLoading ? (
        ...
        <EmptyPanel
          ...
          title="لا يوجد سير رواتب"
          desc="أنشئ أول سير رواتب لهذا الشهر."
        />
      ) : (
        <div className="space-y-4">
          {(q.data ?? []).map((r) => (
            <PayrollRunCard
              ...
              onDelete={() => {
                if (confirm("حذف السير؟")) delMut.mutate(r.id);
              }}
            />
          ))}
        </div>
    ...
    <div className="rounded-2xl border bg-card p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-xs font-semibold ${locked ? "bg-emerald-500/15 text-emerald-700" : "bg-amber-500/15 text-amber-700"}`}
          >
            {locked ? "معتمد" : "مسودة"}
          </span>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <div>
            ...
            <b>{Number(run.total_gross).toLocaleString("ar-SA")}</b>
          </div>
          <div>
            <span className="text-muted-foreground">صافي:</span>{" "}
            <b>{Number(run.total_net).toLocaleString("ar-SA")}</b>
          </div>
          {!locked && (
            <button
              onClick={onFinalize}
          ...
          <tbody>
            {items.map((it) => (
              <PayrollItemRow
                ...
                locked={locked}
                onSave={(v) =>
                  mut.mutate({ id: it.id, allowances: v.allowances, deductions: v.deductions })
                }
              />
            ))}
          </tbody>
