...
export const Route = createFileRoute("/_authenticated/admin/appointments/$id")({
  head: () => ({
    meta: [{ title: "تفاصيل الموعد | لوحة الإدارة" }, { name: "robots", content: "noindex" }],
  }),
  ...
  notFoundComponent: () => (
    <div className="container-app py-16 text-center text-muted-foreground">الموعد غير موجود.</div>
  ),
        ...
        <section className="rounded-lg border p-4">
          ...
          <dl className="mt-2 space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <dd>{a.patient_name ?? "—"}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted-foreground">الجوال</dt>
              <dd className="font-mono text-xs">{a.patient_phone ?? "—"}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted-foreground">البريد</dt>
              <dd className="text-xs">{a.patient_email ?? "—"}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted-foreground">الهوية</dt>
              <dd className="font-mono text-xs">{a.national_id ?? "—"}</dd>
            </div>
          </dl>
        </section>
        <section className="rounded-lg border p-4">
              ...
              {history.map((h: unknown) => (
                <li key={h.id} className="flex justify-between border-b py-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {new Date(h.changed_at).toLocaleString("ar-SA")}
                  </span>
                </li>
              ))}
            </ul>
