...
const getAiOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<AiOverview> => {
    ...
    const [flags, routes, convCount, msgCount, incidents, latest, tools, costs, convs, handoffIncidents] = await Promise.all([
      ...
      context.supabase.from("ai_tool_invocations").select("tool, status, latency_ms, cost_usd").gte("created_at", since).limit(5000),
      context.supabase.from("ai_usage_costs").select("cost_usd").gte("day", today),
      context.supabase.from("ai_conversations").select("id, scope").gte("started_at", since).limit(5000),
      context.supabase.from("ai_safety_incidents").select("conversation_id, kind, action_taken, severity").gte("created_at", since).limit(5000),
    ]);
    ...
    // Tool stats
    const toolMap = new Map<string, { total: number; ok: number; error: number; denied: number; timeout: number; latencies: number[]; cost: number }>();
    let toolCostSum = 0;
    for (const t of tools.data ?? []) {
      const e = toolMap.get(t.tool) ?? { total: 0, ok: 0, error: 0, denied: 0, timeout: 0, latencies: [], cost: 0 };
      e.total += 1;
    ...
    const tool_stats: ToolStat[] = Array.from(toolMap.entries()).map(([tool, e]) => {
      ...
      return {
        tool, total: e.total, ok: e.ok, error: e.error, denied: e.denied, timeout: e.timeout,
        success_rate: e.total ? e.ok / e.total : 0,
        avg_latency_ms: Math.round(avg), p95_latency_ms: Math.round(p95), cost_usd: Number(e.cost.toFixed(4)),
      };
    }).sort((a, b) => b.total - a.total);

    // Overall response latency (across all tools)
    const allLat = (tools.data ?? []).map((t) => t.latency_ms).filter((n): n is number => typeof n === "number").sort((a, b) => a - b);
    const avg_response_ms = allLat.length ? Math.round(allLat.reduce((s, n) => s + n, 0) / allLat.length) : 0;
    const p95_response_ms = allLat.length ? allLat[Math.min(allLat.length - 1, Math.floor(allLat.length * 0.95))] : 0;
    ...
    const handoff_by_scope: HandoffStat[] = Array.from(scopeCount.entries()).map(([scope, conversations]) => {
      const handoffs = handoffConvs.get(scope)?.size ?? 0;
      return { scope, conversations, handoffs, rate: conversations ? handoffs / conversations : 0 };
