...
function NotifLogsPage() {
  ...
  const [liveStatus, setLiveStatus] = useState<"connecting" | "live" | "error" | "off">(
    "connecting",
  );
    ...
    <div className="p-5 space-y-5 text-sm">
      <section className="space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[11px] text-muted-foreground">محاولة #{detail.attempt}</span>
          {canRetry && (
            <button
              type="button"
        ...
        <dl className="grid grid-cols-3 gap-x-3 gap-y-1 text-xs">
          ...
          <dd className="col-span-2">{new Date(detail.created_at).toLocaleString("ar")}</dd>
          <dt className="text-muted-foreground">آخر تحديث</dt>
          <dd className="col-span-2">{new Date(detail.updated_at).toLocaleString("ar")}</dd>
          ...
          <dd className="col-span-2 font-mono break-all">{detail.recipient ?? "—"}</dd>
      ...
      {detail.error_message && (
        <section>
          <h3 className="text-xs font-semibold text-rose-700 mb-1">سبب الفشل</h3>
      ...
      {n && (
        <section className="space-y-2">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase">الرسالة الأصلية</h3>
          <div className="rounded-xl border p-3 space-y-2">
            <div className="flex items-center gap-2 flex-wrap text-[11px] text-muted-foreground">
              <span className="rounded-full bg-slate-100 px-2 py-0.5">kind: {n.kind ?? "—"}</span>
              <span className="rounded-full bg-slate-100 px-2 py-0.5">
                audience: {n.audience ?? "—"}
              </span>
              <span className="rounded-full bg-slate-100 px-2 py-0.5">{n.send_status ?? "—"}</span>
            </div>
            <div>
...
function KpiHeader24h({ k }: { k: NotificationDeliveryKpis }) {
  ...
  return (
    <section aria-label="مؤشرات الأداء خلال 24 ساعة" className="rounded-2xl border bg-white p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-bold text-foreground">آخر 24 ساعة · مؤشرات التسليم</h2>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-xl border bg-slate-50/50 p-3">
          ...
          <div className="text-[10px] text-muted-foreground mt-1">
            نجاح {k.successCount.toLocaleString("ar")} · فشل {k.failedCount.toLocaleString("ar")} ·
            قيد الإرسال {k.pendingCount.toLocaleString("ar")}
          </div>
        </div>

        {/* Avg delivery time by channel */}
        <div className="rounded-xl border bg-slate-50/50 p-3">
          <div className="text-[11px] text-muted-foreground mb-1.5">متوسط زمن التسليم لكل قناة</div>
        ...
        {/* Top failure reasons */}
        <div className="rounded-xl border bg-slate-50/50 p-3">
          <div className="text-[11px] text-muted-foreground mb-1.5">أعلى أسباب الفشل</div>
          {k.topFailureReasons.length === 0 ? (
            ...
            <ul className="space-y-1">
              {k.topFailureReasons.map((r, i) => (
                ...
                >
                  <span className="truncate text-slate-700" title={r.reason}>
                    {r.reason}
                  </span>
...
function LiveBadge({
  ...
  return (
    <span
      ...
      title={
        status === "live" ? `التحديث الفوري مفعّل · ${bump.toLocaleString("ar")} حدث` : cfg.label
      }
      aria-live="polite"
    >
