...
import {
  AlertTriangle, ArrowLeft, CalendarPlus, CheckCircle2, Loader2, RefreshCw,
  Save, ShieldAlert, ShieldCheck, ShieldOff,
} from "lucide-react";
    ...
    <div className="portal-root portal-gradient-bg min-h-dvh" dir="rtl">
      <main className="mx-auto max-w-4xl px-4 sm:px-6 py-6 sm:py-8">
        <header className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-[color:var(--portal-ink)]">التأمين الطبي</h1>
            <p className="text-xs sm:text-sm text-[color:var(--portal-ink-2)]">وثيقتك الحالية وسجل التحقق من الأهلية</p>
          </div>
        </header>
        ...
        <section className="mt-6">
          <h2 className="text-sm font-semibold text-[color:var(--portal-ink-2)] mb-3 px-1">سجل التحقق من الأهلية ({vs.length})</h2>
          {vs.length === 0 ? (
            <EmptyState />
          ) : (
              ...
              {vs.map((v) => (
                <li key={v.id} className="glass-card p-4 sm:p-5">
                  <div className="flex items-start gap-3">
                    <div className={`h-10 w-10 shrink-0 rounded-xl grid place-items-center border ${v.eligible ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-red-50 text-red-600 border-red-100"}`}>
                      {v.eligible ? <CheckCircle2 className="h-5 w-5" /> : <ShieldOff className="h-5 w-5" />}
                    </div>
                      ...
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div>
                          <h3 className="text-sm font-semibold text-[color:var(--portal-ink)]">
                            ...
                            <span className="ms-2 text-[11px] text-[color:var(--portal-ink-2)] font-normal">{v.policy_hint ? `• ${v.policy_hint}` : ""}</span>
                          </h3>
                          {v.message && <p className="mt-1 text-xs text-[color:var(--portal-ink-2)] leading-relaxed">{v.message}</p>}
                        </div>
                        <time className="text-[11px] text-[color:var(--portal-ink-2)]" dateTime={v.created_at}>{dt(v.created_at)}</time>
                      </div>
                      <dl className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                        <Metric label="سعر الاستشارة" value={money(v.consultation_fee)} />
                        <Metric label="نسبة التغطية" value={v.coverage_percent != null ? `${v.coverage_percent}%` : "—"} />
