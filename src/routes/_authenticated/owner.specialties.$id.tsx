...
import {
  getOwnerSpecialty,
  createOwnerSpecialty,
  updateOwnerSpecialty,
...
export const Route = createFileRoute("/_authenticated/owner/specialties/$id")({
  head: () => ({
    meta: [
      { title: "تحرير تخصص · Site Builder" },
...
function SpecialtyEditor() {
  ...
  const getFn = useServerFn(getOwnerSpecialty);
  const createFn = useServerFn(createOwnerSpecialty);
  const updateFn = useServerFn(updateOwnerSpecialty);
  ...
  async function submit() {
    ...
    try {
      ...
      if (isNew) {
        ...
        toast.success("تم إنشاء التخصص");
        navigate({ to: "/owner/specialties/$id", params: { id: row.id } });
      } else {
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Button asChild variant="ghost" size="icon" aria-label="رجوع">
            <Link to="/owner/specialties">
              <ArrowRight className="h-4 w-4" />
            </Link>
          </Button>
          <h1 className="text-2xl font-bold text-slate-900">
            {isNew ? "تخصص جديد" : form.name_ar || "تحرير التخصص"}
          </h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white rounded-xl border shadow-sm p-5">
        ...
        <div>
          <Label>الأيقونة (اسم Lucide)</Label>
          <Input
            dir="ltr"
            value={form.icon}
        ...
        <div className="md:col-span-2 flex items-center justify-between pt-2 border-t">
          <Label>مفعّل (يظهر في الموقع)</Label>
