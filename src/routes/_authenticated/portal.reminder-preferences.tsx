  ...
  return (
    <div className="mx-auto max-w-3xl" dir="rtl">
      <header className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-[color:var(--portal-ink-2)]">
            اختر كيف ومتى تصلك التذكيرات والإشعارات.
          </p>
        </div>
      </header>
      ...
      <form onSubmit={submit} className="glass-card p-5 sm:p-6 space-y-8">
        <Section
          title="قنوات الإشعار"
          hint="اختر قناة واحدة على الأقل. سنستخدم القنوات المفعّلة معًا حسب توفّرها."
        >
          <ChannelToggle
            channelKey="in_app"
            ...
            {(
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[11px] text-[color:var(--portal-ink-2)] mt-1">
                    {opt.hint}
                  </div>
                </button>
              );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="تذكير الموعد قبل (بالدقائق)">
              <input
                ...
                onChange={(e) =>
                  set("appointment_lead_minutes", Math.max(0, Number(e.target.value) || 0))
                }
                className={inputCls}
              />
            </Field>
            <Field label="تذكير الدواء قبل (بالدقائق)">
              <input
                ...
                onChange={(e) =>
                  set("medication_lead_minutes", Math.max(0, Number(e.target.value) || 0))
                }
                className={inputCls}
              />
            </Field>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="بداية النشاط (ساعة اليوم)">
              <input
                ...
                onChange={(e) =>
                  set("wake_hour", Math.min(23, Math.max(0, Number(e.target.value) || 0)))
                }
                className={inputCls}
              />
            </Field>
            <Field label="نهاية النشاط (ساعة اليوم)">
              <input
                ...
                onChange={(e) =>
                  set("sleep_hour", Math.min(23, Math.max(0, Number(e.target.value) || 0)))
                }
                className={inputCls}
              />
            </Field>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-[color:var(--portal-border)]">
          <p className="text-[11px] text-[color:var(--portal-ink-2)] inline-flex items-center gap-1">
            ...
            <Link to="/portal/notifications" className="underline">
              الإشعارات
            </Link>
            .
          </p>
          <div className="flex gap-2">
            <Link
              to="/portal"
              className="h-10 px-4 rounded-full border border-[color:var(--portal-border)] bg-[color:var(--portal-surface)] text-sm inline-flex items-center"
            >
              إلغاء
            </Link>
            <button
