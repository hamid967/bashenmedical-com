...
function DoctorCalendarPage() {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["portal", "doctor-calendar"] });
  ...
  const statusMut = useMutation({
    ...
    onSuccess: (_d, v) => {
      invalidate();
      toast.success(v.status === "cancelled" ? "تم إلغاء الموعد" : "تم تحديث الحالة");
    },
  ...
  const slotStatusMut = useMutation({
    mutationFn: (v: { id: string; status: "available" | "blocked" }) =>
      setMySlotStatus({ data: v }),
    onSuccess: () => {
      invalidate();
  ...
  const byDate = useMemo(() => {
    const map = new Map<
      string,
      { appts: number; booked: number; available: number; blocked: number; onLeave: boolean }
    >();
    const days: string[] = [];
    for (let d = new Date(gridStart); d <= gridEnd; d.setDate(d.getDate() + 1)) days.push(iso(d));
    days.forEach((d) =>
      map.set(d, { appts: 0, booked: 0, available: 0, blocked: 0, onLeave: false }),
    );
    const data = calQuery.data;
    if (data) {
      data.appointments.forEach((a) => {
        const b = map.get(a.appointment_date);
        if (b && (a.status === "new" || a.status === "confirmed" || a.status === "completed"))
          b.appts++;
      });
  ...
  const dayAppts = (calQuery.data?.appointments ?? []).filter(
    (a) => a.appointment_date === selectedDate,
  );
  const daySlots = (calQuery.data?.slots ?? []).filter((s) => s.slot_date === selectedDate);
  const dayLeaves = (calQuery.data?.leaves ?? []).filter(
    (lv) => selectedDate >= lv.start_date && selectedDate <= lv.end_date,
  );
      ...
      <header className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <p className="text-sm text-[color:var(--portal-ink-2)]">
            اعرض مواعيدك وأدر الفترات بسهولة.
          </p>
        </div>
          ...
          <>
            <div className="grid grid-cols-7 gap-1 mb-2">
              {WEEKDAYS.map((w) => (
                <div
                  key={w}
                  className="text-center text-xs font-semibold text-[color:var(--portal-ink-2)] py-1"
                >
                  {w}
                </div>
              ))}
                  ...
                  cells.push(
                    <button
                      ...
                      className={[
                        ...
                        isSelected
                          ? "ring-2 ring-[color:var(--portal-accent)] border-[color:var(--portal-accent)]"
                    ...
                    >
                      <div className="flex items-center justify-between text-xs">
                        <span
                          className={isToday ? "font-bold text-[color:var(--portal-accent)]" : ""}
                        >
                          {d.getDate()}
                        </span>
                        {b?.onLeave && (
                          <span title="إجازة" className="text-amber-500">
                            ✕
                          </span>
                        )}
                      </div>
            ...
            <div className="mt-3 flex flex-wrap gap-3 text-xs text-[color:var(--portal-ink-2)]">
              <span className="inline-flex items-center gap-1">
                <span className="h-2.5 w-2.5 rounded bg-emerald-500" /> موعد مؤكّد
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="h-2.5 w-2.5 rounded bg-teal-500" /> فترة متاحة
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="h-2.5 w-2.5 rounded bg-slate-400" /> محجوب
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="h-2.5 w-2.5 rounded bg-amber-500" /> إجازة
              </span>
            </div>
          </>
        )}
            ...
            {dayAppts.length === 0 ? (
              ...
              <ul className="space-y-2">
                {dayAppts.map((a) => (
                  <li
                    key={a.id}
                    className="p-3 rounded-lg border border-slate-200 bg-[color:var(--portal-surface)]"
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {a.status !== "confirmed" &&
                        ...
                        a.status !== "cancelled" && (
                          <ActionBtn
                            onClick={() => statusMut.mutate({ id: a.id, status: "confirmed" })}
                      ...
                      {a.status !== "cancelled" && a.status !== "completed" && (
                        <>
                          ...
                          <ActionBtn
                            onClick={() => statusMut.mutate({ id: a.id, status: "no_show" })}
                            disabled={statusMut.isPending}
                          >
                            عدم حضور
                          </ActionBtn>
          ...
          <section>
            ...
            {daySlots.length === 0 ? (
              <p className="text-sm text-[color:var(--portal-ink-2)]">
                ...
                <Link
                  to="/portal/schedule"
                  className="text-[color:var(--portal-accent)] hover:underline"
                >
                  أضف فترات ←
                </Link>
              </p>
              ...
              <ul className="space-y-2">
                {daySlots.map((s) => (
                  <li
                    key={s.id}
                    className="p-2 rounded-lg border border-slate-200 bg-[color:var(--portal-surface)] flex items-center gap-2"
                  >
                    ...
                    {s.status === "available" && (
                      <>
                        <ActionBtn
                          onClick={() => slotStatusMut.mutate({ id: s.id, status: "blocked" })}
                          disabled={slotStatusMut.isPending}
                        >
                          <Ban className="h-3.5 w-3.5" /> حجب
                        </ActionBtn>
                        <ActionBtn
                          onClick={() => {
                            if (confirm("حذف هذه الفترة؟")) slotDeleteMut.mutate(s.id);
                          }}
                          disabled={slotDeleteMut.isPending}
                          tone="danger"
                        >
                          حذف
                        </ActionBtn>
                    ...
                    {s.status === "blocked" && (
                      <ActionBtn
                        onClick={() => slotStatusMut.mutate({ id: s.id, status: "available" })}
                        disabled={slotStatusMut.isPending}
                        tone="success"
                      >
...
function RescheduleDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={onClose}>
      <div
        dir="rtl"
        className="bg-[color:var(--portal-surface)] rounded-xl shadow-xl w-full max-w-sm p-5"
      ...
      >
        ...
        <div className="mt-5 flex gap-2 justify-end">
          <button className="portal-btn portal-btn-ghost" onClick={onClose} disabled={pending}>
            إلغاء
          </button>
          <button
...
function PatientCalendarPage() {
  ...
  const q = useQuery({
    ...
    queryFn: () => listMyAppointments({ data: { scope: "all", fromDate, toDate, limit: 100 } }),
    staleTime: 30_000,
  });
    ...
    <div dir="rtl" className="space-y-6">
      <header className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1 min-w-[220px]">
          ...
          <p className="text-sm text-[color:var(--portal-ink-2)]">
            نظرة شهرية على مواعيدك مع تنقّل سريع لأي يوم.
          </p>
        </div>
        <Link
      ...
      <div className="glass-card p-4 sm:p-5">
        ...
        {q.isLoading ? (
          ...
          <>
                  ...
                  cells.push(
                    <button
                      ...
                      className={[
                        ...
                        inMonth ? "bg-white" : "bg-transparent opacity-50",
                        isSelected
                          ? "ring-2 ring-[color:var(--portal-primary)] border-[color:var(--portal-primary)]"
                          : "border-slate-200 hover:border-slate-300",
                    ...
                    >
                      <div className="flex items-center justify-between text-xs">
                        <span
                          className={isToday ? "font-bold text-[color:var(--portal-primary)]" : ""}
                        >
                          {d.getDate()}
                        </span>
                      </div>
            ...
            <div className="mt-3 flex flex-wrap gap-3 text-xs text-[color:var(--portal-ink-2)]">
              <span className="inline-flex items-center gap-1">
                <span className="h-2.5 w-2.5 rounded bg-emerald-500" /> مؤكّد
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="h-2.5 w-2.5 rounded bg-teal-500" /> جديد
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="h-2.5 w-2.5 rounded bg-red-500" /> ملغى
              </span>
            </div>
          </>
        )}
      ...
      <div className="glass-card p-4 sm:p-5">
        <h2 className="text-base font-bold mb-4">
          مواعيد{" "}
          {new Date(`${selectedDate}T00:00:00`).toLocaleDateString("ar-SA-u-nu-latn", {
            weekday: "long",
            day: "numeric",
            month: "long",
            year: "numeric",
        ...
        {dayAppts.length === 0 ? (
          <p className="text-sm text-[color:var(--portal-ink-2)]">
            ...
            <Link to="/portal/book" className="text-[color:var(--portal-primary)] hover:underline">
              احجز موعدًا ←
            </Link>
          </p>
        ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-[color:var(--portal-ink-2)] mt-0.5 flex flex-wrap gap-x-3 gap-y-1">
                      {a.branch && (
                        <span className="inline-flex items-center gap-1">
                          <MapPin className="h-3 w-3" /> {a.branch.name_ar}
                        </span>
                      )}
                    ...
                    {a.reason && (
                      <div className="text-xs text-[color:var(--portal-ink-2)] mt-1 line-clamp-2">
                        <StickyNote className="h-3 w-3 inline-block mr-1" />
                        {a.reason}
                      </div>
                    )}
                  </div>
