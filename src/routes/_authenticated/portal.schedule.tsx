...
export const Route = createFileRoute("/_authenticated/portal/schedule")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "لوحة تحكم الطبيب: إدارة الجدول الأسبوعي، الإجازات، وفترات المواعيد.",
      },
    ...
    return (
      <div className="glass-card max-w-lg mx-auto p-8 text-center">
        ...
        <p className="text-sm text-[color:var(--portal-ink-2)] mt-2">
          إن كنت طبيبًا في المجمع، يرجى التواصل مع الإدارة لربط حسابك بسجلك الطبي حتى تتمكن من إدارة
          جدولك.
        </p>
        <Link
          to="/portal"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi
          icon={Clock}
          label="أوقات متاحة (مقبلة)"
          value={counts.availableSlots}
          tone="emerald"
        />
        <Kpi
          icon={CalendarClock}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 items-end p-4 rounded-2xl bg-[color:var(--portal-gradient-soft)] mb-5">
        ...
        <div>
          ...
          <input
            ...
            max={120}
            value={gBreak}
            onChange={(e) => setGBreak(Number(e.target.value))}
            className="mt-1 w-full h-10 rounded-lg border bg-[color:var(--portal-surface)] px-2 text-sm"
          />
        </div>
        <button
              ...
              <ul className="flex flex-wrap gap-2">
                {list.map((s) => {
                  ...
                  return (
                    <li
                      key={s.id}
                      className={`inline-flex items-center gap-2 rounded-full ${tone} px-3 py-1.5 text-xs`}
                    >
                      <span dir="ltr" className="font-mono">
...
function LeavesSection() {
  ...
  const createMut = useMutation({
    mutationFn: () =>
      createFn({ data: { startDate, endDate, allDay: true, reason: reason.trim() || null } }),
    onSuccess: () => {
  ...
  return (
    <section className="glass-card p-6">
      ...
      {leaves.length === 0 ? (
        <div className="text-xs text-[color:var(--portal-ink-3)] text-center py-4">
          لا توجد إجازات
        </div>
      ) : (
              ...
              <div className="mt-1 flex items-center gap-2 text-xs">
                ...
                {a.patient_phone && (
                  <a
                    ...
                    className="text-[color:var(--portal-primary)]"
                    dir="ltr"
                  >
                    {a.patient_phone}
                  </a>
